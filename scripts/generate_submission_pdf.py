...
def build_pdf(output_path):
    ...
    arch_data = [
        ...
        [Paragraph("<b>Application API Layer</b>", styles['TableCellBold']), Paragraph("FastAPI (Python 3.11+), Uvicorn, Pydantic v2", styles['TableCell']), Paragraph("REST endpoints and multipart ingestion parsed/indexed synchronously in the request; no background worker or retry queue. `project_id` is a filter, not authorization; no application auth/RBAC is implemented, so deploy behind an authenticated gateway.", styles['TableCell'])],
        ...
        [Paragraph("<b>RAG & Advisory Planner</b>", styles['TableCellBold']), Paragraph("LangGraph + Sentence Transformers (BGE/Cross-Encoder)", styles['TableCell']), Paragraph("Query rewriting and classification (`knowledge`, `compliance`, `schedule`), RRF score fusion, and evidence gating. Suggested services/endpoints are advisory only; Copilot continues through retrieval without dispatching them.", styles['TableCell'])],
        [Paragraph("<b>AI Gateway & LLM</b>", styles['TableCellBold']), Paragraph("Groq API (`openai/gpt-oss-120b`) + Google Gemini (`2.0-flash`)", styles['TableCell']), Paragraph("Ultra-fast structured entity extraction, submittal compliance explanations, and cited response formulation.", styles['TableCell'])],
    ]
    ...
    story.append(Paragraph("• <b>Step 1 — Query Rewriting & Advisory Classification:</b> The planner analyzes the user query and active equipment thread context, expanding abbreviations and classifying it as document search (`knowledge`), compliance (`compliance`), or schedule (`schedule`). It can recommend an existing service/endpoint, but does not dispatch it; Copilot continues through retrieval.", styles['BulletCustom']))
    ...
    story.append(Paragraph("<b>Module E: Synthetic Supply-Chain Demo & Counterfactual Mitigation Calculator</b>", styles['SubSectionHeading']))
    story.append(Paragraph("The synthetic supply-chain demo uses project-supplied CSV milestones and synthetic scenarios; no live carrier, AIS, vendor, ERP, or position feed is connected. When a critical-path delay scenario is available, the counterfactual mitigation calculator applies user-supplied recovery and cost assumptions deterministically. Its values are not predictions or quotations:", styles['BodyTextCustom']))
    
    mit_data = [
        [Paragraph("Counterfactual Action", styles['TableHeader']), Paragraph("Configured Recovery Assumption", styles['TableHeader']), Paragraph("User-Supplied Cost Assumption ($)", styles['TableHeader']), Paragraph("Residual Risk & Tradeoffs", styles['TableHeader'])],
        [Paragraph("<b>Scenario 1: Expedite Supplier Recovery Plan</b>", styles['TableCellBold']), Paragraph("14 days", styles['TableCell']), Paragraph("15,000", styles['TableCell']), Paragraph("Low-risk scenario assumption; not a freight quotation or commitment.", styles['TableCell'])],
        [Paragraph("<b>Scenario 2: Re-sequence On-Site Electrical Crews</b>", styles['TableCellBold']), Paragraph("10 days", styles['TableCell']), Paragraph("8,500", styles['TableCell']), Paragraph("Medium-risk scenario assumption; requires engineering review before action.", styles['TableCell'])],
        [Paragraph("<b>Scenario 3: Utilize Weather Contingency Windows</b>", styles['TableCellBold']), Paragraph("5 days", styles['TableCell']), Paragraph("0", styles['TableCell']), Paragraph("High-risk scenario assumption; absorbs the configured weather buffer.", styles['TableCell'])],
    ]
    ...
    story.append(Paragraph("• <b>Project-Scoped Filtering, Not Authorization:</b> Database rows and vector points carry `project_id` filters, but this is not tenant authorization. Application authentication and RBAC are not implemented; deploy the service behind an authenticated gateway.", styles['BulletCustom']))
    story.append(Paragraph("• <b>Synchronous Ingestion Today:</b> Document parsing and indexing run in the request; no background worker or retry queue is running. Queue-backed ingestion remains future roadmap work rather than an active capability.", styles['BulletCustom']))
    story.append(Paragraph("• <b>Live Enterprise Adapters (Roadmap):</b> The current submission uses synthetic data only: no live carrier, AIS, vendor, ERP, or position feed is connected. Future integration work may consider Primavera P6, SAP ERP, and live logistics feeds.", styles['BulletCustom']))
    story.append(Paragraph("• <b>Application Authentication/RBAC (Roadmap):</b> No application authentication or role-based access control is implemented. A future role model may distinguish engineering, QA, and commissioning responsibilities; until then, use an authenticated gateway.", styles['BulletCustom']))
    story.append(Spacer(1, 15))
